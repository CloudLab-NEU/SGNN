...
The [output_recevied_value.zip](json%2Foutput_recevied_value.zip) file needs to be unzipped. 
...
## Files in .json/
The [input_sent_value.json](json%2Finput_sent_value.json) and [output_recevied_value.zip](json%2Foutput_recevied_value.zip) are available from [BigQuery](https://cloud.google.com/bigquery/public-data) based on txId2hash.csv.
...
## Run
Running [train_address.py](..%2Ftrain%2Ftrain_address.py) requires the 6 .npz files for [addr0_tx_mean] recorded in [path.conf](..%2Fconfig%2Fpath.conf) and [address_inOrOut_tx_time_class.csv](process%2Faddress_inOrOut_tx_time_class.csv).
